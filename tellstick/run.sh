...
for (( i=0; i < "$DEVICES"; i++ )); do
    ...
    ATTR_GROUP=$(jq --raw-output ".devices[$i].group // 0" $CONFIG_PATH)
    ...
    (
        ...
        if [ ! -z "$ATTR_HOUSE" ] || [ ! -z "$ATTR_CODE" ] || [ ! -z "$ATTR_UNIT" ] || [ ! -z "$ATTR_FADE" ] || [ ! -z "$ATTR_GROUP" ]; then
            echo "  parameters {"
            ...
            if [ ! -z "$ATTR_GROUP" ]; then
                echo "    group = \"$ATTR_GROUP\""
            fi
